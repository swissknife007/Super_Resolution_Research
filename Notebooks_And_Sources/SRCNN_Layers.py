...
class Conv_Layer_BN_ReLu(object):
    #Batch norm implementation from theano-users forum
    ...
    def __init__(self, rng, input, filter_shape, image_shape):
        ...
        conv1_len = image_shape[2] - filter_shape[2] + 1 

        gamma1 = theano.shared(np.ones((image_shape[0],filter_shape[0],conv1_len,conv1_len), dtype=theano.config.floatX),borrow=True)
        beta1 = theano.shared(np.zeros((image_shape[0],filter_shape[0],conv1_len,conv1_len), dtype=theano.config.floatX),borrow = True)
        mean1 = conv_out.mean(0, keepdims=True)
        std1 = conv_out.std(0, keepdims=True)
        std1 = T.sqrt(std1**2 + 1e-6) # for stability

        conv_out = conv_out + self.b.dimshuffle('x', 0, 'x', 'x')
        conv1_bn = batch_normalization(inputs=conv_out, gamma=gamma1, beta=beta1,
                                                  mean=mean1, std=std1)
        self.output = T.nnet.relu(conv1_bn)

        # store parameters of this layer
        self.params = [self.W, self.b, gamma1, beta1]
