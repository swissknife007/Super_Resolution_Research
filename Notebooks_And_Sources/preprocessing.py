...
def create_subimages(image_folder, output_folder, output_type = 'YCbCr',upsampling_factor = 4):
            ...
            with open(output_folder + '_filelist_sub_imgs.txt','w') as write_file:
                for counter,image_filename in enumerate(sorted(filenames)):
                    if image_filename.split('.')[-1] == 'bmp' and image_filename[0] != '.':
                        ...
                        for w in range(int((image.shape[0]-33)/14) + 1):
                            for h in range(int((image.shape[1]-33)/14) + 1):
    ...
    print("mapping file: ", output_folder + "mapping")
...
def upsample(data_x):
    ...
    for i in range(data_x.shape[0]):
        ...
        temp=transform.resize(image=img, output_shape=(33,33,3), order=0, mode='constant')
        upsampled_x[i,:,:,:]=np.transpose(temp,(2,0,1)) 
...
if __name__ == '__main__':
    ...
    create_subimages('/Users/himaniarora/Desktop/Columbia/Courses/Neural_networks/project/SRCNN/project/Data/Training_Full',
        '/Users/himaniarora/Desktop/Columbia/Courses/Neural_networks/project/SRCNN/project/Data/temp',
