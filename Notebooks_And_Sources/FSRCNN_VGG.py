...
def train_FSRCNN_VGG(train_set_x,train_set_y,valid_set_x,valid_set_y,test_set_x,test_set_y,
    ...
    model_output_clipped = model_output#T.clip(model_output,0.0,1.0)
    
    model_output_copied = model_output_clipped
    
    mat = scipy.io.loadmat('Layers1to7_16vgg.mat')
    
    im_dims=np.asarray([16, 16],dtype=int)
    ...
    im_dims=np.asarray([33, 33],dtype=int)
    print('... building the vgg model for high resolution input')
    print('im_dims=', im_dims)
    
    layer1_high_input = reshaped_gt 
    ...
    MSE_per_pixel = cost2/(output_len_low*output_len_low*128)
    ...
    reconstucted_imgs = model_output_clipped
    ...
    decay_learning_rate_function = theano.function([],learning_rate,updates = [(learning_rate,learning_rate * 0.99)])

    train_nn_vgg(train_model, validate_model, test_model,
