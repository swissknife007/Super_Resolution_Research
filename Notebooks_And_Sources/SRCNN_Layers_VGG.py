...
def train_nn(train_model, validate_model, test_model,
    ...
    while (epoch < n_epochs) and (not done_looping):
        ...
        for minibatch_index in range(n_train_batches):
            ...
            [cost_ij,mse_ij,pnsr_ij,pnsr_orig_ij,reconstucted_imgs] = train_model(minibatch_index)
            
            if (iter % 100 == 0) and verbose:
                print('training @ iter = ' + str(iter) + "\tcost = " + str(cost_ij) + 
                    "\tmse/pixel = " + str(mse_ij) +
                    "\t pnsr = " + str(pnsr_ij)+
                    "\t pnsr_orig = " + str(pnsr_orig_ij))
            ...
            if (iter + 1) % validation_frequency == 0:
                ...
                for i in range(n_valid_batches):
                    ...
                    validation_pnsr_orig.append(valid_output[3])
                    validation_reconstructed.append(valid_output[4])
                    
                
                this_validation_loss = np.mean(validation_losses)

                
                print('epoch %i, minibatch %i/%i, validation cost %f mse/pixel: %f pnsr: %f pnsr_orig: %f' %
                        (epoch,
                         minibatch_index + 1,
                         n_train_batches,
                ...
                if this_validation_loss < best_validation_loss:
                    ...
                    for i in range(n_test_batches):
                        ...
                        test_pnsr_orig.append(test_output[3])
                        test_reconstructed.append(test_output[4])
                    ...
                    print(('     epoch %i, minibatch %i/%i, test cost of '
                               'best model %f perpixel mse %f and test pnsr %f, orig_psnr %f') %
                              (epoch, minibatch_index + 1,
                               n_train_batches,
                               test_score,
                               np.mean(test_MSE),
                               np.mean(test_pnsr),
                               np.mean(test_pnsr_orig)))
