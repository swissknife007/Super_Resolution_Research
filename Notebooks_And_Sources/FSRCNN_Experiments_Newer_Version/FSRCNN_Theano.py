...
def train_FSRCNN_PRelu(train_set_x,train_set_y,valid_set_x,valid_set_y,test_set_x,test_set_y,
    n_train_batches, n_valid_batches, n_test_batches, n_epochs, batch_size,lr,upsampling_factor=4):
    #Assume x to be shape (batch_size,3,33,33)
    x = T.matrix('x')
    y = T.matrix('y')
    ...
    #Conv for Patch extraction
    #print('batch size', batch_size)
    conv1 = Conv_Layer_PReLU(rng, reshaped_input, image_shape=(batch_size,3,input_image_size,input_image_size),filter_shape = (64,3,f1,f1));
    conv1_len = input_image_size 
    #Conv for Non linear mapping
    #print('conv1 done....')
    conv2 = Conv_Layer_PReLU(rng, conv1.output, image_shape=(batch_size,64,conv1_len,conv1_len),filter_shape = (32,64,f2,f2))
    conv2_len = conv1_len
    #Conv for Reconstruction
    #conv2_output =  conv2.output.repeat(2,2)
    #conv2_output =  conv2_output.repeat(2,3)
...
def train_FSRCNN3(train_set_x,train_set_y,valid_set_x,valid_set_y,test_set_x,test_set_y,
    n_train_batches, n_valid_batches, n_test_batches, n_epochs, batch_size,lr,upsampling_factor=4):
    #Assume x to be shape (batch_size,3,33,33)
    x = T.matrix('x')
    y = T.matrix('y')
    ...
    #PSNR of a patch is based on color space
    MSE_per_pixel = (1.0/batch_size * T.sum((sub_y-model_output) ** 2))/(output_len*output_len*3)
    psnr = 20 * T.log10(255) - 10 * T.log10(MSE_per_pixel)
    reconstucted_imgs = model_output
...
def train_FSRCNN2(train_set_x,train_set_y,valid_set_x,valid_set_y,test_set_x,test_set_y,
    n_train_batches, n_valid_batches, n_test_batches, n_epochs, batch_size,lr,upsampling_factor=4):
    #Assume x to be shape (batch_size,3,33,33)
    x = T.matrix('x')
    y = T.matrix('y')
    ...
    conv2_output =  conv2.output.repeat(2,2)
    conv2_output =  conv2_output.repeat(2,3)
    conv3 = Conv_Layer_ReLU(rng, conv2_output, image_shape=(batch_size,32,conv2_len*2,conv2_len*2),filter_shape = (3,32,f3,f3))
    model_output = conv3.output
    '''
    conv3 = De_Conv_Layer_ReLU2(rng, conv2.output, image_shape=(batch_size,32,conv2_len,conv2_len),filter_shape = (3,32,f3,f3))
    model_output = conv3.output
    '''
    #print(model_output.shape)
    #grab center pixels
